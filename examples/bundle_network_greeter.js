// Example: Greeter Service Pack (Isomorphic)
...
module.exports.default = async function(context) {
  ...
  const intervalId = setInterval(async () => {
    ...
    // Add Node.js-specific diagnostics when available
    if (typeof process !== 'undefined' && process.memoryUsage) {
      payload.nodeVersion = process.version;
      payload.memoryMB = Math.round(process.memoryUsage().heapUsed / 1024 / 1024 * 100) / 100;
    }

    console.log(`[${serviceId}] → Sending greeting #${seq} to echo-service (uptime ${now - startTime}ms${payload.memoryMB ? ', mem ' + payload.memoryMB + 'MB' : ''})`);

    try {
      const sendTime = Date.now();
      // Standard fetch() — Stark intercepts *.internal URLs automatically
